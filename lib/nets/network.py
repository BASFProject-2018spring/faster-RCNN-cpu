...
class Network(nn.Module):
  ...
  def _anchor_target_layer(self, rpn_cls_score):
    ...
    rpn_labels = Variable(torch.from_numpy(rpn_labels).float()) #.set_shape([1, 1, None, None])
    rpn_bbox_targets = Variable(torch.from_numpy(rpn_bbox_targets).float())#.set_shape([1, None, None, self._num_anchors * 4])
    rpn_bbox_inside_weights = Variable(torch.from_numpy(rpn_bbox_inside_weights).float())#.set_shape([1, None, None, self._num_anchors * 4])
    rpn_bbox_outside_weights = Variable(torch.from_numpy(rpn_bbox_outside_weights).float())#.set_shape([1, None, None, self._num_anchors * 4])
  ...
  def _anchor_component(self, height, width):
    ...
    self._anchors = Variable(torch.from_numpy(anchors))
  ...
  def forward(self, image, im_info, gt_boxes=None, mode='TRAIN'):
    ...
    self._image = Variable(torch.from_numpy(image.transpose([0,3,1,2])), volatile=mode == 'TEST')
    self._im_info = im_info # No need to change; actually it can be an list
    self._gt_boxes = Variable(torch.from_numpy(gt_boxes)) if gt_boxes is not None else None
  ...
  def extract_head(self, image):
    feat = self._layers["head"](Variable(torch.from_numpy(image.transpose([0,3,1,2])), volatile=True))
    return feat
